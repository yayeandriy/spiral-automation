...
function processNestedBlocks(allBlocks, parentId) {
  ...
  for (const childBlock of directChildren) {
    if (childBlock.type === 'column_list') {
      ...
      if (columns.length > 0) {
        // Calculate column widths using actual width ratios from Notion
        const columnWidths = columns.map(column => {
          const widthRatio = column.column?.width_ratio;
          if (widthRatio && typeof widthRatio === 'number') {
            return Math.round(widthRatio * 100 * 100) / 100; // Round to 2 decimal places
          }
          // Fallback to equal distribution if no width_ratio
          return Math.floor(100 / columns.length);
        });
        
        // Ensure total width is 100% by adjusting the last column if needed
        const totalWidth = columnWidths.reduce((sum, width) => sum + width, 0);
        if (totalWidth !== 100 && columnWidths.length > 0) {
          columnWidths[columnWidths.length - 1] += (100 - totalWidth);
        }
        
        // Generate colgroup with actual widths

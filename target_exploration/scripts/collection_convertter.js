...
function pickDateValue(value) {
  ...
  // For date ranges, prefer end date over start date
  if (value.end) {
    return value.end;
  }
...
function extractDate(entry) {
  ...
  if (entry?.properties) {
    const explicitKeys = ['timeline', 'Timeline', 'publish_date', 'PublishDate', 'Publish date', 'published', 'Date'];
...
function extractAreas(entry) {
  const raw = entry?.property_areas || entry?.areas;
  if (Array.isArray(raw)) {
    return raw.map(area => {
  ...
  if (entry?.properties) {
    const areasProperty = entry.properties.areas
      || entry.properties.Areas
      || Object.values(entry.properties).find(prop => 
        prop?.type === 'multi_select' && (prop?.name === 'areas' || prop?.name === 'Areas')
      );

    if (areasProperty?.multi_select) {
      return areasProperty.multi_select.map(option => option?.name || '').filter(Boolean);
    }
  }
...
const articles = processedEntries
  .map(({ entry, rawDate }) => {
    ...
    const areas = extractAreas(entry);
    ...
    if (areas.length > 0) {
      const areaTags = areas.map(area => `<span class="tag">${escapeHtml(area)}</span>`).join(' ');
      lines.push(`      <div class="meta-item">${areaTags}</div>`);
    }
